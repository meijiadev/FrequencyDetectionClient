...
object FileUtil {
    ...
    fun getLogCacheDir(ctx: Context = MyApp.appContext): String {
        val fileDirPath = ctx.externalCacheDir.toString() + "/FrequencyDetectionClient"
    ...
    fun getAudioCacheDir(ctx: Context = MyApp.appContext): String {
        val fileDir = ctx.getExternalFilesDir(Environment.DIRECTORY_MUSIC) ?: return " "
        return fileDir.parent
    }

    fun getAlarmCacheDir(ctx: Context = MyApp.appContext): String {
        val fileDir = ctx.getExternalFilesDir(Environment.DIRECTORY_ALARMS) ?: return " "
        return fileDir.parent
    }
    ...
    fun getFileSize(filePath: String?, @ByteUnit.ByteUnitDef unit: String) =
        getFileSize(getFileByPath(filePath), unit)
