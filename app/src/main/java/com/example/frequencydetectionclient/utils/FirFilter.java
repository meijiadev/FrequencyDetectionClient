 ...
 * Description:此类实现FIR滤波器。大部分代码复制自GNU Radio的firdes和firfilter模块。
 */
public class FirFilter {
										  ...
										  float attenuation_dB)   // attenuation dB
	{
		if (sampling_freq <= 0.0) {
			Logger.e(LOGTAG,"createLowPass: firdes check failed: sampling_freq > 0");
			return null;
		}

		if (cutoff_freq <= 0.0 || cutoff_freq > sampling_freq / 2) {
			Logger.e("createLowPass: firdes check failed: 0 < fa <= sampling_freq / 2");
			return null;
		}

		if (transition_width <= 0) {
			Logger.e(LOGTAG,"createLowPass: firdes check failed: transition_width > 0");
			return null;
		}
