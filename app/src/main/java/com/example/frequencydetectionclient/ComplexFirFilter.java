...
public class ComplexFirFilter {
										  ...
										  float attenuation_dB)   // attenuation dB
	{
		if (sampling_freq <= 0.0) {
			Logger.e("createBandPass: firdes check failed: sampling_freq > 0");
			return null;
		}

		if (low_cutoff_freq < sampling_freq * -0.5 || high_cutoff_freq > sampling_freq * 0.5) {
			Logger.e("createBandPass: firdes check failed: -sampling_freq / 2 < fa <= sampling_freq / 2");
			return null;
		}

		if (low_cutoff_freq >= high_cutoff_freq) {
			Logger.e(LOGTAG,"createBandPass: firdes check failed: low_cutoff_freq >= high_cutoff_freq");
			return null;
		}

		if (transition_width <= 0) {
			Logger.e(LOGTAG,"createBandPass: firdes check failed: transition_width > 0");
			return null;
		}
		...
		for(int i = 0; i < ntaps; i++) {
			...
			phase += freq;
			//Logger.d("createBandPass: Filter Taps [i="+i+"]: " + tapsReal[i] + "   " + tapsImag[i]);
		}
